...
**验证。** `cd rust && cargo fmt`（九个文件需要重新排版——这是私有 main 分支本就有的既存格式漂移，不是这次同步引入的）之后 `cargo build --locked --workspace` 成功。`cargo clippy --workspace --all-targets --keep-going`：73 条告警，与本仓库记录的基线相同（本次同步没有引入任何新告警）。`cargo test --locked --workspace`：**1033 通过，0 失败，10 忽略**（上面那处夹具修复之后；修复前的第一次跑，唯一的失败正好是那处路径夹具的测试）。

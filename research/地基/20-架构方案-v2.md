...
- **B143–B147（`地基/附注/2026-09-25-批5裁定.md`，2026-09-25）**：§3.8 键空间段加 B143 注（记录 `model` 只写真实来源、固定端口 id 取夹具声明）；§2.5 批调度行加 B145 注（检视点判据、运行期 `StateHash` 分组）。B144、B146、B147 只改 `12` 与 `21`。
...
- **B148–B151（搭配层：JEV × 生成器 × 执行器 × 图算法，`地基/比赛/附注-2026-09-25-搭配层设计与36小时施工.md`，2026-09-25）**：§2.1 L6 行加 `actions/` 模块；§2.3 L6「crate 形态」加生成器端口目录与 `actions/`；§2.5 批调度行加 B149 注（同层 `gen`/`judge`/`do` 一次提交、生成不阻塞）；§3.6 加搭配层原语返回契约值与 `interval` 区间一段；§五 加 S12（新增生成器端口）、S13（新增搭配层库骨架）、S14（新增宿主动作）三行。条文本体在 `12` §2.3 B148、§2.4 B149、§2.5 B150、§5 B151；施工步在 `21` §四·14（15h、24e-1、24e-2、19 追加项、25-8a、25c、25d、25e）。
- **B153（读数算术：`cut` 的 `stat` 与声明式拟合，`地基/附注/2026-09-26-批6裁定.md` §一，2026-09-26）**：§3.4 等级表 `Declared` 行的来源扩为「`cut` 的 `declare`、`stat` 声明线、声明式拟合 `fit({declare})`」，`releases()` 与 `host_accepts_declared` 不改；§2.4 J-01 行不改（读数与 `Score` 在程序里不是数）；J-08 静态子面来源穿过 `Score`。条文本体在 `12` §2.3 B153 补、§2.9 声明式拟合段；施工步 `21` 20j-3、20j-4。
- **B154（JEV `confidence` 作可选统计量，同附注 §二，2026-09-26）**：§2.3 `jpp-effects` 判断答案加可选 `confidence`；§九账本行判断条目加可选字段（随 18a）；画像 schema 加 H9 `reports_confidence`。条文本体在 `12` §1.2 H9、§2.3 B154 补、§2.10。
- **B155（候选随题走、按材料合批，同附注 §三，2026-09-26）**：§2.5 批调度行 B145 注后加一句（分组键为材料哈希）；§2.3 L6 `backends/jev/` 线上形状（state JSON 只发 `on`/`ctx`/`ref`，`over` 作题的 `criteria`）；`RENDER_VERSION` `r2`。条文本体在 `12` §2.1 B155 条、§2.2 B155 注、§2.10 线上形状行；施工步 `21` 15i。
- **B156（子集请求与重排；J-04 不改，同附注 §四，2026-09-26）**：§3.4 正交位表加 `crosstalk_untested` 行；画像 schema 加 H8 子项 `subset_crosstalk`。条文本体在 `12` §2.2 子集句 B156 注、§3 J-04 行注；施工步 `21` 15j。
- **B157（确定性文本与数据内置；Q18 直绑，同附注 §五，2026-09-26）**：§2.1/§2.3 L1 `jpp-ir` 表达式形加 `Builtin(name)`（只由装载器对库作用域生成）；L4 `jpp-runtime` 内置表加文本与数据组；`now` 不作内置（D13.1 不变）。条文本体在 `12` §2.7 B157 注；施工步 `21` 7c、25-4b。
- **B160–B163（施工口径：15h 两段与按层提交、`compose` 累加 α 与 `cert` 读法、J-05 按键计一次、`spent` 按登记归属，同附注 §八–§十一，2026-09-26）**：§2.5 批调度行不改（`gen` 随层提交已在 B149 注内）；§3.4 合成出口加 `alpha_bound`、`n_unknown` 字段；§3.6 契约值 `spent` 口径一句；`Exit` 读法内置 `cert`。条文本体在 `12` §2.4 B160 注、§2.3 B161 注、§5 搭配层行、§3 J-05 B162 注、§2.12 B163 注。
- **B164–B166（执行器可逆位 = 沙箱内、线的两端开闭、`order` 单读数分档，同附注 §十二–§十四，2026-09-26）**：§五 S14 事实行加 `sandbox: {kind, fs, net, undo}` 与派生 `reversible`；§2.3 L6 `actions/` 加沙箱探测；§3.4 声明线记录加 `closed`；`order` 签名加单读数支。条文本体在 `12` §2.5 B164、§2.3 B165、§2.2 B166。
- **B167（`order` 加 `stat`，同附注 §十五，2026-09-26）**：§2.3 `jpp-value` 加 `Stat` 枚举与 `stat_of`（`cut`、`order`、声明式拟合共用）；`order` 签名加 `{stat?, tie?}`；打分读数缺省排序键为档位。条文本体在 `12` §2.2 B167 注。
...
| L6 | `jpp` | 宿主接口与命令行，一个 crate 带 lib 与 bin 两个目标：`session/`（facade：`Session`、`compile`/`explain`/`run`/`replay`/`resume`/`feed_calib`，不读写固定路径）、`store/`（持久对象：料库、题库、画像装载、缓存索引、存储后端、迁移，原 `jpp-store`）、`backends/<name>/`（真机端口，feature 门控；`backends/jev/` 只在 feature `live` 下编译，原 `jpp-backend-jev`）、`actions/`（宿主动作表：执行器 `exec_py`/`exec_sql`/`check_tests`、精确算法 `graph:*`、检索 `embed_topk`/`bm25_topk`，各带可逆位与 `taint_out`，`BUILTIN_ACTIONS` 事实表在此；B150，2026-09-25）、`cli/`（参数、文件路径、报告落盘、真机开关，bin 目标，原 `jpp-cli`）〔B74 改：原文本行只是 facade（600 行），另有 `jpp-store`（L4，1,600 行）、`jpp-backend-jev`（L5，700 行）、`jpp-cli`（L7，1,200 行）三行三个 crate；依 Nature 2026-09-23 授权，Fable 裁定 B74（地基/附注/2026-09-24-评估①裁定.md）〕 | 库 API；进程入口 | 4,100 |
...
**crate 形态。** lib 目标承载 `session/`、`store/`、`backends/`；bin 目标承载 `cli/`。feature `live` 打开 `backends/jev/` 与可选依赖 `ureq`；不开时只有固定观察、零调用、重放三个内置端口（`jpp-effects`）。〔B149、B150（2026-09-25）：生成器端口 `backends/<gen>/`（第一个是子进程 `claude -p`）与判断器端口同为 lib 目标下的模块目录，实现 `EffectPort` 且**非阻塞**——`submit` 交线程池立即返回票据、`poll` 查完成、并发从画像读，注册表一行、`--gen-model` 一处；`actions/` 为 lib 目标下的宿主动作模块（执行器、精确算法、检索），`BUILTIN_ACTIONS` 事实表移到此处，`cli/runner.rs` 只引用；两者都不进内核依赖表。〕对外包名是 `jpp`。
...
|---|---|---|---|---|---|---|
| 批调度 | `jpp-plan`（`lift`、`fuse`、`speculate`、`vectorize`、`schedule` 写 `SitePlan`/`TriggerPlan`） | `PlanHooks::instantiate`（拿到方法值后补目标站点，实现在 `jpp-plan`） | 标注 + 触发点关系 | `jpp-runtime/register.rs`、`flush.rs`、`schedule.rs` | 无：程序无并发数、无「立即发」语法 | `16` §一的十四倍写法与高阶骨架，经 `TriggerPlan.dynamic` 消除；纸面推演见 §4.5，实测归 `tests/equiv_pairs`。另一种：多题 `sieve` 返回契约值列表时，作者按下标把结果对回填法、手工合并 K 份 `pending`（探针 folio 行 50–57、winnow-batched 行 15–22）——调用数没绕过，写法绕过了封闭；经 B82 单一契约值消除（§3.6），仪表以调用数比与逐行归类取数（B80）。第三种：直线段里经函数包装的逐题 `judge; cut; handle`（align 非设计者版 36 次对 9 次）与 `cut` 立即刷新使同层两次 `sieve` 各占一层（winnow 四层对两层）——经 B94 `cut` 惰性与 `lift` 穿过直线段里的函数调用消除（`21` 步 23c）。第四种：递归逐题（每步检视后再递归）按构造串行，语言不消除，由 B97 的工具让作者改用 `sieve`/`judge(state, [题])`〔依 Nature 2026-09-23 授权，Fable 裁定 B94、B97（地基/评估/2026-09-24-仪表读数4诊断与裁定.md）〕 〔B145（2026-09-25）：检视点的判据是「凡结果依赖于出口的操作」，B94 六项是例子；提升分组按运行期 `StateHash`；递归只挡一层属「不保证合批」〕〔B155（2026-09-26）：分组键改为材料哈希 `hash(on, ctx, ref)`，同材料候选集不同的 `select` 合成一次调用；线上 state JSON 不含 `over`。依 Nature 2026-09-23 授权，Fable 裁定 B155（地基/附注/2026-09-26-批6裁定.md §三）〕 〔B149 注（2026-09-25）：同一层内待发的 `gen`、`judge`、`do` 由 `schedule` 一次提交、轮询取回；生成器端口非阻塞，生成期间无依赖的判断、执行、算法照常发出，账本条目按登记序；生成器慢只拖长它自己那条依赖链〕 |
...
| `crosstalk_untested` | 出口来自子集请求（`request: "all"`，一次请求 K 个单元）而画像 `subset_crosstalk` 未测或为真（B156，2026-09-26） | 不改 | 置位则不放行不可逆 `do`，路由可用；画像测为假即不置位 | `W-subset-untested` | B156 |
...
**搭配层原语的返回值（B148，2026-09-25）。** `search`、`verify`、`on_graph` 返回契约值，形状同本节；`judged_graph` 返回 `Graph = {nodes, edges: accepted 元素, rejected, pending: 未决边元素, unobserved, spent}`，其 `edges`/`pending` 是 `pair` 元素（带 `left`/`right`/`at`/`exit`）；`interval` 返回 `{lo: Outcome, hi: Outcome, differs: [未决边元素], alpha_sum: Float}`，`lo`/`hi` 元素的出口是所用边出口经 `compose(…, "all")` 的合成，`differs` 非空时该产物 `releases()` 为假（未决边参与了决定）。嵌套时内层 `pending` 经 `carry` 进外层 `pending`（`trail` 加一层），J-05 的运行期检查不变。读数不进这些结构的任何数值字段；边权只允许非判断来源的量。
...
| S12 新增生成器端口（B149） | `jpp/src/backends/<gen>/`（`EffectPort` 非阻塞实现：线程池 `submit`/`poll`）、画像 JSON 一份（`gen` 分表：`cost_usd_per_call`、`latency_p95_ms`、`concurrency`、`failure_kinds`、`empty_rate`、`taint_out` 缺省 `untrusted`） | `backends/mod.rs` 注册 1 行；`jpp::cli` 的 `--gen-model` 1 处 | 内核 L0–L4 全部；`schedule.rs` 不按效应种类分支 | 画像缺字段 = `Untested`；端口 `submit` 内同步等待由 15e 的并发测试（同层 `gen` 与 `judge` 完成时间重叠）拦下 |
| S13 新增搭配层库骨架（B148） | `jpp-lib/compose/<name>.jpp`（写在 `sieve`/`pair`/`first_k`/`order`/`iterate`/`compose`/`do` 之上）、金样与 `tests/equiv_pairs` 一对 | 依据文本 `12` §5 搭配层行加一项 | 内核 L0–L4（原语不得使用 `.jpp` 拿不到的内核能力，否则走 S3） | `trace.py` 核 `lib/compose/*.jpp` 每个有指向 `12` §5 的 `// 依据：`；账本 `calls_per_layer` 断言层内一次刷新 |
| S14 新增宿主动作（B150） | `jpp/src/actions/<group>.rs`（闭包 + 事实行） | `actions/mod.rs` 的 `BUILTIN_ACTIONS` 1 行 | 内核 L0–L4；`jpp-check`（J-08/J-11 只读事实表） | 事实行缺失 → 注册时 `expect` 失败；固定观察下动作由夹具供出，缺即 `unserved`；不可逆动作未守卫由 J-08 静态子面报 |
